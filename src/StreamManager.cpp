...
void
parse_stream_definition(const std::string& text, std::vector<std::string>& result)
{
...
public:
  ...
  void write(const Issue& issue)
  {
    ...
    if (m_manager.m_out_streams[s].get() == this) {
      m_manager.m_out_streams[s] = std::shared_ptr<OutputStream>(m_manager.setup_stream(s));
    }
...
void
ers::StreamManager::add_receiver(const std::string& stream, const std::string& filter, ers::IssueReceiver* receiver)
{
...
{
  std::scoped_lock lock(m_mutex);
  for (std::list<std::shared_ptr<InputStream>>::iterator it = m_in_streams.begin(); it != m_in_streams.end();) {
