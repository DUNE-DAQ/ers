...
ERS_DECLARE_ISSUE(ers, InvalidFormat, "Creator for the \"" << key << "\" stream is not found", ((std::string)key))
...
public:
  ...
  InputStream* create_in_stream(const std::string& stream, const std::initializer_list<std::string>& params)
    const; /**< \brief create new stream */
...
std::ostream&
operator<<(std::ostream&, const ers::StreamFactory&);
}
