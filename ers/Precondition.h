...
protected:
	...
	Precondition(ers_severity s, const Context &context); 
public:
	static const char* PRECONDITION_CLASS ; 
	Precondition();
	Precondition(const char*condition, const std::string &message, ers_severity s, const Context &context, bool constant_expression=false) ;
...
#ifdef __GNUC__
#define ERS_PRECONDITION(expr,...) { if(!expr) { char precondition_buffer[256] ; snprintf(precondition_buffer,256, __VA_ARGS__) ; ers::Precondition failed_precondition(#expr,precondition_buffer,ers::ers_error,ERS_HERE,__builtin_constant_p(expr)) ; throw failed_precondition ; } }
#else 
#define ERS_PRECONDITION(expr,...) { if(!expr) { char precondition_buffer[256] ; snprintf(precondition_buffer,256, __VA_ARGS__) ; ers::Precondition failed_precondition(#expr,precondition_buffer,ers::ers_error,ERS_HERE,false) ; throw failed_precondition ; } }
#endif
