...
getTimeFormatter()
{
  static std::string format = ers::read_from_environment("DUNEDAQ_ERS_TIMESTAMP_FORMAT", "%Y-%b-%d %H:%M:%S");

  static const std::string precision = ers::read_from_environment("DUNEDAQ_ERS_TIMESTAMP_PRECISION", "MILLI");

  static const bool isUTC = ::getenv("DUNEDAQ_ERS_TIMESTAMP_UTC");

  if (boost::algorithm::ifind_first(precision, "NANO")) {
    return [](const ers::Issue& issue) { return issue.time<std::chrono::nanoseconds>(format, isUTC); };
  }

  if (boost::algorithm::ifind_first(precision, "MICRO")) {
    return [](const ers::Issue& issue) { return issue.time<std::chrono::microseconds>(format, isUTC); };
  }

  if (boost::algorithm::ifind_first(precision, "MILLI")) {
    return [](const ers::Issue& issue) { return issue.time<std::chrono::milliseconds>(format, isUTC); };
  }

  return [](const ers::Issue& issue) { return issue.time(format, isUTC); };
}
...
ers::StandardStreamOutput::print(std::ostream& out, const Issue& issue, int verbosity)
{
  ...
  if (verbosity > 1) {
    ...
    for (std::vector<std::string>::const_iterator it = issue.qualifiers().begin(); it != issue.qualifiers().end();
         ++it) {
      out << "'" << *it << "' ";
    }
  }
  ...
  if (verbosity > 2) {
    out << FIELD_SEPARATOR << "host = " << issue.context().host_name() << FIELD_SEPARATOR
        << "user = " << issue.context().user_name() << " (" << issue.context().user_id() << ")" << FIELD_SEPARATOR
        << "process id = " << issue.context().process_id() << FIELD_SEPARATOR
        << "thread id = " << issue.context().thread_id() << FIELD_SEPARATOR << "process wd = " << issue.context().cwd();
  }
