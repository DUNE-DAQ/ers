...
namespace
{
    ...
    {
	librespy.def("init", &init);
	librespy.def("debug", &debug);
	librespy.def("log", &log);
	librespy.def("info", &info);
	librespy.def("warning", &warning);
	librespy.def("error", &error);
	librespy.def("fatal", &fatal);
    }
}
