...
private:
  ...
  void set_receiver(IssueReceiver* receiver) { m_receiver = receiver; }
...
#define ERS_REGISTER_INPUT_STREAM(class, name, params)                                                                 \
  ...
  {                                                                                                                    \
    static ers::InputStream* create(const std::initializer_list<std::string>& params) { return new class(params); }    \
    InputStreamRegistrator() { ers::StreamFactory::instance().register_in_stream(name, create); }                      \
  } registrator_mp;                                                                                                    \
  }
