...
public:
  ...
  virtual Context* clone() const /**< \return copy of the current context */ { return new RemoteContext(*this); }

  const char* cwd() const /**< \return current working directory of the process */ { return m_process.m_cwd.c_str(); }

  const char* file_name() const /**< \return name of the file which created the issue */ { return m_file_name.c_str(); }
  ...
  int line_number() const /**< \return line number, in which the issue has been created */ { return m_line_number; }

  const char* package_name() const /**< \return CMT package name */ { return m_package_name.c_str(); }

  pid_t process_id() const /**< \return process id */ { return m_process.m_pid; }

  pid_t thread_id() const /**< \return thread id */ { return m_process.m_tid; }

  void* const* stack_symbols() const /**< \return stack frames */ { return 0; }

  int stack_size() const /**< \return number of frames in stack */ { return 0; }

  int user_id() const /**< \return user id */ { return m_process.m_uid; }

  const char* user_name() const /**< \return user name */ { return m_process.m_uname.c_str(); }

  const char* application_name() const /**< \return application name */ { return m_process.m_app_name.c_str(); }
