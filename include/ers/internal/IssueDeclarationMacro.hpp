...
#define ERS_ATTRIBUTE_NAME_TYPE(_, __, tuple) , ERS_TYPE(tuple) ERS_NAME(tuple)

#define ERS_ATTRIBUTE_SERIALIZATION(_, __, tuple) set_value(BOOST_PP_STRINGIZE(ERS_NAME(tuple)), ERS_NAME(tuple));
...
#define __ERS_DECLARE_ISSUE_BASE__(namespace_name, class_name, base_class_name, message, base_attributes, attributes)  \
  ...
  protected:                                                                                                           \
    BOOST_PP_EXPR_IF(                                                                                                  \
      BOOST_PP_NOT_EQUAL(BOOST_PP_SEQ_SIZE(base_attributes attributes), 0), class_name(const ers::Context& context)    \
      : base_class_name(context) { ; })                                                                                \
  public:                                                                                                              \
    static const char* get_uid() { return BOOST_PP_STRINGIZE( namespace_name::class_name ); }                          \
    class_name(const ers::Context& context ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)          \
                 ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes));                                       \
    class_name(const ers::Context& context,                                                                            \
               const std::string& msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)               \
                 ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes));                                       \
    class_name(const ers::Context& context,                                                                            \
               const std::string& msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)               \
                 ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes),                                        \
...
#define __ERS_DEFINE_ISSUE_BASE__(                                                                                     \
  INLINE, namespace_name, class_name, base_class_name, message, base_attributes, attributes)                           \
  namespace namespace_name {                                                                                           \
  INLINE class_name::class_name(const ers::Context& context ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE,                    \
                                                                           ERS_EMPTY base_attributes)                  \
                                  ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes))                       \
    : base_class_name(context ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME, ERS_EMPTY base_attributes)){ ERS_PRINT_LIST(          \
        ERS_ATTRIBUTE_SERIALIZATION,                                                                                   \
        ERS_EMPTY attributes) BOOST_PP_EXPR_IF(BOOST_PP_NOT(ERS_IS_EMPTY(ERS_EMPTY message)),                          \
                                               ERS_SET_MESSAGE(ERS_EMPTY message)) }                                   \
                                                                                                                       \
    INLINE class_name::class_name(                                                                                     \
      const ers::Context& context,                                                                                     \
      const std::string& msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)                        \
        ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes))                                                 \
    : base_class_name(context, msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME, ERS_EMPTY base_attributes)){ ERS_PRINT_LIST(     \
        ERS_ATTRIBUTE_SERIALIZATION,                                                                                   \
        ERS_EMPTY attributes) }                                                                                        \
                                                                                                                       \
    INLINE class_name::class_name(                                                                                     \
      const ers::Context& context,                                                                                     \
      const std::string& msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)                        \
      ...
      const std::exception& cause)                                                                                     \
    : base_class_name(context,                                                                                         \
                      msg ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME, ERS_EMPTY base_attributes),                               \
                      cause){ ERS_PRINT_LIST(ERS_ATTRIBUTE_SERIALIZATION, ERS_EMPTY attributes) } INLINE               \
    class_name::class_name(                                                                                            \
      const ers::Context& context ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY base_attributes)                   \
        ERS_PRINT_LIST(ERS_ATTRIBUTE_NAME_TYPE, ERS_EMPTY attributes),                                                 \
      const std::exception& cause)                                                                                     \
...
#define ERS_DECLARE_ISSUE_BASE_HPP(namespace_name, class_name, base_class_name, message, base_attributes, attributes)  \
  __ERS_DECLARE_ISSUE_BASE__(                                                                                          \
    namespace_name, class_name, base_class_name, ERS_EMPTY message, ERS_EMPTY base_attributes, ERS_EMPTY attributes)

#define ERS_DEFINE_ISSUE_BASE_CXX(namespace_name, class_name, base_class_name, message, base_attributes, attributes)   \
  __ERS_DEFINE_ISSUE_BASE__(ERS_EMPTY,                                                                                 \
                            namespace_name,                                                                            \
                            class_name,                                                                                \
                            base_class_name,                                                                           \
                            ERS_EMPTY message,                                                                         \
