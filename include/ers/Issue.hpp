...
{
public:
  IssueRegistrator() { ers::IssueFactory::instance().register_issue(T::get_uid(), create); }

private:
  static ers::Issue* create(const Context& context) { return new T(context); }
};
...
public:
  ...
  const Issue* cause() const /**< \brief return the cause Issue of this Issue */ { return m_cause.get(); }

  const Context& context() const /**< \brief Context of the issue. */ { return *(m_context.get()); }

  const std::string& message() const /**< \brief General cause of the issue. */ { return m_message; }

  const std::vector<std::string>& qualifiers() const /**< \brief return array of qualifiers */ { return m_qualifiers; }

  const string_map& parameters() const /**< \brief return array of parameters */ { return m_values; }

  ers::Severity severity() const /**< \brief severity of the issue */ { return m_severity; }
  ...
  const system_clock::time_point& ptime() const /**< \brief original time point of the issue */ { return m_time; }

  const char* what() const noexcept /**< \brief General cause of the issue. */ { return m_message.c_str(); }
...
std::ostream&
operator<<(std::ostream&, const ers::Issue&);
} // ers
...
ers::Issue::time(const std::string& format, bool isUTC) const
{
  ...
  auto c = std::chrono::duration_cast<Precision>(m_time.time_since_epoch()).count();
